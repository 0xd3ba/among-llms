...
class GameStateManager:
    ...
    def __invalid_response_by_agent(self, agent_id: str, try_num: int) -> None:
        """ Callback to notify the chat that the agent generated an invalid response """
        title = f"Invalid Response Generated by {agent_id}"
        msg_prefix = f"[{try_num}/{AppConfiguration.max_model_retries}]"

        if try_num < AppConfiguration.max_model_retries:
            message = f"{msg_prefix} Will retry again shortly ..."
        else:
            message = f"{msg_prefix} Max. retries exceeded. Skipping the turn ..."

        self.__invoke_chat_callback(
            ChatCallbackType.NOTIFY_TOAST,
            title=title,
            message=message,
            severity=ToastConfiguration.type_error,
            timeout=2.0
        )
    ...
    def __generate_callbacks(self) -> dict[StateManagerCallbackType, Callable[..., Any]]:
        ...
        self_callbacks = {
            ...
            StateManagerCallbackType.INVALID_RESPONSE: self.__invalid_response_by_agent,
        }
